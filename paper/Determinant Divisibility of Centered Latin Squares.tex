...
(empirically sharp: over nine sampled orders up to $n=20$ the bound is never
violated, with equality attained in each sampled $(n,p)$ pair),
...
($5n^2$ mixing steps each), $10.56\%$ satisfy
...
for $n=8$, $10.56\%$ of Jacobson--Matthews samples achieve
...
interval) always yields $\rank_{\FF_2}(\mathcal{P}_{T_n})=n/2+1$.
This is because
...
drops from~$n$ by $k-1$ to $n-(k-1)=n/2+1$.  The skip-one
modification $S_n=T_n\setminus\{k-1\}\cup
...
$(n,p,k_p)$ triple: $2700$ $(n,p,\mathrm{sample})$ bound tests on
Jacobson--Matthews samples across the sampled
orders $n\in\{6,8,9,10,12,15,16,18,20\}$ and, for each order, the primes
$p\mid n$ (so $p\in\{2,3,5\}$), produced zero violations, and equality with
the bound was attained by at least one sample in each $(n,p)$ pair (though
not by every sample, nor at every $k_p$ level).
\end{remark}
...
available in the public open-source repository (the \texttt{scripts/} and \texttt{results/} trees)
...
switch-chain table (seed~$20260412$), the cyclic Smith-normal-form table for
$n\le 12$, the $p$-adic bound scan over the sampled orders
$n\in\{6,8,9,10,12,15,16,18,20\}$, $p\in\{2,3,5\}$, and the \Cref{ex:n10} witness JSON certificate;\ and
...
\begin{enumerate}[label=\textup{(\arabic*)}]
  \item \textbf{Density of counterexamples.}
        ...
        At $n=8$, $10.56\%$ of \emph{Jacobson--Matthews
